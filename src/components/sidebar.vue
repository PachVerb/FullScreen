      ...
      <div class="side-list">
        <div
          :class="[
            'side-item',
            'side-bar-item-aa',
            sideItem.children ? 'side-item-has-child' : '',
          ]"
        ...
        >
          <div
            class="sys-tips"
            :style="{
              top: sideItem.children
                ? -10 + sideItem.children.length * -55 + 'px'
                : 0,
            }"
            v-if="sideItem.children"
          >
            <div class="sys-tips-body">
              <span
                :class="[
                  'sys-sidechild-normal',
                  checkSideItem.name === currentSysModule
          ...
          <div
            :class="[
              'side-normal',
              sideItem.name === currentSys ? 'side-select' : '',
            ]"
          >
  ...
  methods: {
    ...
    checkSideList(type) {
      ...
      if (type == "up" && this.thisIndex > 0) {
        ...
        type == "down" &&
        this.thisIndex < Math.floor(this.allSideList.length / 6)
      ) {
        this.thisIndex += 1;
      }
    ...
    hoverCurrentSys(side, index) {
      ...
      if (side && side.children) {
        ...
        this.SysLeft =
          30 + (315 - (midIndex - index) * 105) - marginLeft + "px";
      }
    },
    ...
    checkCurrentSysModule(sideChild, parentSys) {
      if (
        this.currentSys !== parentSys.name &&
        this.$parent.$refs[this.currentSys] &&
        this.$parent.$refs[this.currentSys].destroySys
      )
        this.$parent.$refs[this.currentSys].destroySys();
...
.right-side-item {
  animation: right-side-item-ani 1.6s cubic-bezier(0.25, 0.46, 0.45, 0.94) both;
}
...
.left-side-item {
  animation: left-side-item-ani 1.6s cubic-bezier(0.25, 0.46, 0.45, 0.94) both;
}
