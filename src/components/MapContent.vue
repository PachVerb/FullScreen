  ...
  methods: {
    ...
    Map() {
      ...
      this.$nextTick(() => {
        ...
        this.lineData = this.getLineRoute([
          [
            [
              [104.04438225523825, 30.590859031757148],
              [104.0520259740045, 30.590965706491346],
            ],
            [
              [104.0520259740045, 30.590965706491346],
              [104.064137435021, 30.591233205127864],
            ],
          ],
          [
            [
              [104.0641336053904, 30.591121119890843],
              [104.05203039721283, 30.59079531591118],
            ],
            [
              [104.05203039721283, 30.59079531591118],
              [104.0443878828446, 30.590648538762807],
            ],
          ],
          [
            [
              [104.04867384106075, 30.603724579183023],
              [104.04841078534338, 30.596635156415786],
            ],
            [
              [104.04841078534338, 30.596635156415786],
              [104.04804555382111, 30.59041450122902],
            ],
            [
              [104.04804555382111, 30.59041450122902],
              [104.04801719955606, 30.587295394378316],
            ],
          ],
          [
            [
              [104.06900014634311, 30.58796169547024],
              [104.06903411684874, 30.590745048843033],
            ],
            [
              [104.06903411684874, 30.590745048843033],
              [104.06931015449743, 30.596805608709175],
            ],
            [
              [104.06931015449743, 30.596805608709175],
              [104.07116915682354, 30.59681221243629],
            ],
            [
              [104.07116915682354, 30.59681221243629],
              [104.07119505594596, 30.59971363197485],
            ],
            [
              [104.07119505594596, 30.59971363197485],
              [104.07346099244108, 30.599619326447836],
            ],
          ],
        ]);
        ...
        this.vMap.on("moveend", (e) => {
          let features = this.vMap.queryRenderedFeatures(
            this.vMap.project(this.vMap.getCenter())
          );
          if (!features[0] || !features[0].hasOwnProperty("source")) return;
          if (
            features[0].source != "outdoor" ||
            features[0].properties.hasOwnProperty("building:levels")
          ) {
        ...
        var SelectFloor = new creeper.FloorSelector(this.vMap, {
    ...
    getBuildingAndRouteFn() {
      ...
      for (var i = 0; i < pageData.features.length; i++) {
        if (
          pageData.features[i].geometry.type == "Polygon" &&
          pageData.features[i].properties.newid == "14"
        ) {
    ...
    loadWater() {
      ...
      let water_3D = threeLayer.threemap.objects.water(
        waterdata,
        waterpng,
        0xffffff
      );
      ...
      this.vMap.flyTo({
        ...
        easing: (t) => {
          ...
          if (t == 1) {
            ...
            let buildingdata = that.buildingdata.filter(
              (item) => !ids.includes(item.id)
            );
            let building_3D = threeLayer.threemap.objects.building(
              buildingdata,
              buildingpng
            );
            ...
            that.buildingdata.forEach((e) => {
              if (e.center && textArr.includes(e.text)) {
                ...
                let label = threeLayer.threemap.objects.add2DLabel(div, [
                  ...e.center,
                  e.height + 0.5,
                ]);
        ...
        .then((group) => {
          ...
          let animatecar = () => {
            ...
            for (let n = 0; n < carlist.length; n++) {
              let pointcar = threeLayer.threemap.projectToWorld(
                this.lineData[n][counter][0]
              );
              //console.log(pointcar);
              carlist[n].position.set(pointcar.x, pointcar.y, 0);
              carlist[n].rotation.set(
                0,
                0,
                (-this.lineData[n][counter][1] * Math.PI) / 180 - Math.PI / 2
              );
            }
        ...
        easing: (t) => {
          if (t == 1) {
            setTimeout(() => {
              ...
              setTimeout(() => {
                //外围建筑
                let buildingout = threeLayer.threemap.objects.buildingoutside(
                  that.buildingoutcircle,
                  [104.05768654235203, 30.59122228071986]
                );
