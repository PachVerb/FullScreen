...
export default {
    ...
    currentSys(val) {
      if (this.map) {
        ...
        this.$nextTick(() => {
          ...
          document.querySelector(".sys-left").style.zoom = 0.8;
          document.querySelector(".sys-right").style.zoom = 0.8;
        });
  ...
  methods: {
    ...
    createMarker() {
      ...
      `;
      let marker = new creeper.Marker({ element: div })
        .setLngLat(this.detailMsg.location)
        .addTo(this.map);
